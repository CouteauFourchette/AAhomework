...
def top_titles
  ...
  high_rated_movies = Movie.where('score >= 9.0')
  high_rated_movies.select(:id, :title)
end

def star_wars
  #display the id, title and year of each Star Wars movie in movies.
  # hint: use 'select' and 'where'
  star_war_movies = Movie.where('title LIKE ?', "%Star Wars%")
  star_war_movies.select(:id, :title, :yr)
end
...
def below_average_years
  ...
  Movie.where('score < 5.0').select("yr, COUNT(id) as bad_movies").group(:yr).order("COUNT(id)")
...
def pulp_fiction_actors
  ...
  Actor.select('DISTINCT actors.id', 'actors.name').joins(:castings).joins(:movies).where('movies.title = ?', 'Pulp Fiction')
end
...
def uma_movies
  ...
  Movie.select('DISTINCT movies.id', 'movies.title', 'movies.yr').joins(:castings).joins(:actors).where('actors.name = ?', 'Uma Thurman')
end
